...
class MaceAgent(Agent):
    ...
    class Husband_height(Enum):
        ...
        Moroccan = [177.6, 7.6]
        Hindustan = [174.5, 6.9]
        Other = [183.8, 7.1] #TODO: placeholder
    ...
    def get_med_usage(self):
        if (self.medication_gluc == 0):
            if (self.gluc >= 7) or (self.gluc_value(1) >= 7.78):
    ...
    def get_cessation(self):
        if (self.age >= 16) and (self.age <= 40):
            return 0.0327
        elif (self.age >= 41) and (self.age <= 60):
            return 0.0231
        elif (self.age < 16):
            return 0.0327
        else: #elderly
    ...
    def __init__(self, unique_id, model, ethnicity, age, height, income):
        ...
        self.tolerance_of_diff = self.random.uniform(self.model.tolerance_of_diff[0], self.model.tolerance_of_diff[1])
        ...
        self.susceptability = self.random.uniform(model.susceptibility[0], model.susceptibility[1])
    ...
    def caloric_change(self, calories):
        '''no underweight people plz'''
        metabolic_rate = self.expenditure * ((alpha[self.age] + (beta[self.age] * self.weight)) + self.SEE * SEE[self.age])/0.9
        if self.random.uniform(0, 1) < self.susceptability:
            self.caloric_intake = max(metabolic_rate, self.caloric_intake + calories)  
            
    def school_intervention(self, fraction, weight_reduction, months):
        self.susceptability = max(0, self.susceptability * fraction)
    ...
    def weight_loss(self):
        ...
        self.weight_to_lose = self.random.uniform(1,4) #(self.weight * 0.05)
    ...
    def birth(self):
        ...
        a.mother = self
        self.model.id += 1 # self.model.next_id doesn't work properly
    ...
    def step(self):
        ...
        if self.model.schedule.steps % 12 == self.birthday_month:
            ...
            # Will I start smoking this year?
        ...
        #TODO: Lisette, continue working on this            
        if self.bmi != None:
            ...
            # can only decide yearly if they need medication or not
