...
from sensitivity_test_agent import MaceAgent, fine_and_gray, breslow, Ethnicity_multiplier
import numpy as np
import math
...
class MaceModel(Model):
        ...
        for k, v in ethnicities_num.items():
            ...
            for _ in range(v):
                ...
                if age >= 18:
                    ...
                    #TODO: this could be improved, but is it worth it?? ie forloop through all women that have an age that is > 18 and < 45 + age child
                    if age < 45+12:
                        mommys.append(a)
                else:
                    num_children += 1
            ...
            #giving children < 12 year olds mothers
            ...
            smoking = dict()  
            for keyz, valuez in adult_ethnicity_temp.items():
                #has smoked, has not smoked, is smoking
                ...
                for ethnicity in ethnicity_temp.keys():
                    if len(ethnicity_temp[ethnicity]) == 0:
                        ...
                        while True:
                            ...
                            #TODO: healthy_bmis can get empty?
                            ...
                            if ethnicity_temp == adult_ethnicity_temp:
                                ...
                                if (smoking_stat == 0):
                                    ...
                                    agent.starts_smoking()
            #give women aged between 30-45 children
            ...
            while children_counter != 0:           
                while True:
                    ...
                    if mommy.number_children < 3:
                        ...
                        mommy = None
                # stop condition: can't find a fitting mom.
                if (mommy == None):
                    break                    
    ...
    def __init__(self, n, bmi25_dis_age_adults, bmi25_dis_age_children, \
        bmi_dis_adults, bmi_dis_children, income, smoking_age_adults, smoked_adults, \
            ...
            age_groups = {(0, 110): 1}, interventions = {}, scenario="standard smoking", \
                age_difference = 10, tolerance_of_diff = (0.04, 0.1), \
                    susceptibility = (0, 1), intention = 1, immigration = 1, *args, **kwargs):
        ...
        #sensitivity test
        self.age_difference = age_difference
        self.tolerance_of_diff = tolerance_of_diff
    ...
    def befriending(self, agent, candidate, friends_needed):
        ...
        # remove candidates with an age difference too big
        if (agent.unintended_wl) or (candidate.unintended_wl) or (agent == candidate) or abs(candidate.age - agent.age) > self.age_difference or (candidate in agent.nodes) or candidate.dead or not (agent.tolerate(candidate)) or not (candidate.tolerate(agent)):
        ...
        if candidate.ethnicity == agent.ethnicity and friends_needed[0] > 0 and \
            ...
            # For the Dutch candidate it has to be 2: other
            # agent | candidate
            # Turkish | Dutch
            # Moroccan | Dutch
            # Other | Dutch
        ...
        #0: homogeneity, 1: Dutch, 2: other
            ...
            if (agent.ethnicity == "Dutch") and (candidate.current_friends[1] >= candidate.friends_eth[1]):
                ...
                return friends_needed, 0
            # Dutch | Turkish, Moroccan, Other
            # Turkish | Moroccan, Other
            # Moroccan | Turkish, Other
            # Other | Turkish, Moroccan
            friends_needed[2] -= 1
        else:
            return friends_needed, 0
    ...
    def remove_surplus_friends(self):
        ...
        for agent in self.schedule.agents:
            ...
            FIRE = agent.current_friends
                
            if (FIRE[0] > agent.friends_eth[0]) or \
            (FIRE[1] > agent.friends_eth[1]) or \
            (FIRE[2] > agent.friends_eth[2]):
    ...
    def gephi_export(self):
        ...
        for agent in self.schedule.agents:
            ...
            data_edges["target"].append([node.unique_id for node in agent.nodes])
          
        data_nodes = pd.DataFrame.from_dict(data_nodes)
        data_edges = pd.DataFrame.from_dict(data_edges)
        data_edges = data_edges.explode("target")
    ...
    def worst_community(self, minimum = 1):
        ...
        def MACE_risk(age, income, ethnicity):
            ...
            x = sum(x)
            return ((1-np.exp(-np.cumsum(breslow * np.exp(x)))[119]) * Ethnicity_multiplier[ethnicity].value) #119 is 10 jaar
    ...
    def datacollection(self):
            ...
            for agent in self.schedule.agents:
                ...
                if (agent.age < 18) and (agent.age >= 12):
                    ...
                    elif (agent.MACE_risk  >= 0.1) and (agent.MACE_risk < 0.2):
            ...
            dict = {"Year": float(self.schedule.steps // 12),
                                                       ...
                                                       "low MACE risk mean age": mean(low_mace_risk_age) if len(low_mace_risk_age) > 0 else np.nan,
                                                       "mid MACE risk mean age": mean(mid_mace_risk_age) if len(mid_mace_risk_age) > 0 else np.nan,
                                                       "high MACE risk mean age": mean(high_mace_risk_age) if len(high_mace_risk_age) > 0 else np.nan,
                                                       ...
                                                       "average MACE risk": mean(avg_mace_risk) if len(avg_mace_risk) > 0 else np.nan,
                                                       "Average bmi": mean(bmis) if len(avg_mace_risk) > 0 else np.nan
                                                       }
    ...
    def export_datacollection(self, name):
        ...
        print(df)
        pd.to_pickle(df, "D:/juju/lumc/model/pickle/help" + str(name) + ".pkl")  
